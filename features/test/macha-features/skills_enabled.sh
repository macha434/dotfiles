...
check "claude-skills.json が SHARE に置かれている" \
    bash -c '[ "$(jq -r ".[0].name" '"$SHARE"'/claude-skills.json)" = lean ] \
             && [ "$(jq -r ".[0].repo" '"$SHARE"'/claude-skills.json)" = macha434/lean ] \
...
check "claude plugin lean が入っている" \
    bash -c '/home/vscode/.local/bin/claude plugin list --json \
             | jq -e "any(.[]; .id == \"lean@macha434-plugins\")" >/dev/null'
